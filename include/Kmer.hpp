...
/**
 * 
 * The following lookup tables and reverse complement code is taken from Jellyfish 
 ...
 */
    ...
    static constexpr R bitmask(unsigned int const onecount) {
        return (onecount == 0) ? 0 : (static_cast<R>(-(onecount != 0)) & (static_cast<R>(-1) >> ((sizeof(R) * CHAR_BIT) - onecount)));
    }

    // table that contains bit patterns to mask out the top bits of a word.
    // The table is such that maskTable[k] will mask out the top (64 - 2*k) bits of the word.
    ...
    // from : https://stackoverflow.com/questions/31952237/looking-for-a-constexpr-ceil-function
    ...
    static int64_t doEncodeBinary(char c) {
...
static char decodeBinaryGATB(uint64_t n) {
    switch (n) {
    case 0:
        return 'A';
    case 1:
...
static char decodeBinary(uint64_t n) {
    switch (n) {
    case 0:
        return 'A';
    case 1:
     ...
     * 
     * Following the encoding suggested [here](https://www.biostars.org/p/113640/), originally 
     * suggested by G. Rizk:
     * A : 0
     * C : 1
     * G : 3
    ...
    static uint64_t charToBitsGATB(char c) {
        // Convert to uppercase https://stackoverflow.com/questions/10688831/fastest-way-to-capitalize-words
        return static_cast<uint64_t>( (((c & ~0x20) >> 1) & 0x03) + ((c & 0x08) >> 3) );
    }

    /**
     * Raw encoding of a character c as an integer.  Assumes that c is a valid nucleotide
     **/
    static uint64_t doEncodeBinaryGATB(char c) {
        return charToBitsGATB(c);
    }

    /**
     * Places the binary encoding of the nucleotide in `code'.  Returns
     * true of c is in {A, C, T, G, a, c, t, g} and false if c is in {N, n}
     **/
    static bool encodeBinyGATB(char c, uint64_t &code) {
        code = charToBitsGATB(c);
        return code < 4;
    }

static std::string stringRevComp(const std::string& s) {
  int32_t l = static_cast<int32_t>(s.length());
    std::string r(l, 'X');
    auto rc = [] (char c) -> char {
        switch (c) {
            case 'A':
...
/**
* From https://www.biostars.org/p/113640/.  This only works for a given word right now;
...
class Kmer {
  ...
  public:
    ...
    template <typename ViewT, typename = typename std::enable_if<has_length<ViewT>::value, void>::type>
    Kmer(ViewT& v) { fromChars(v); }

    template <typename IterT, typename = typename std::enable_if<!has_length<IterT>::value, void>::type>
    Kmer(IterT v) { fromChars(v); }

    Kmer(const Kmer& other) = default;

    template <typename IterT, typename = typename std::enable_if<!has_length<IterT>::value, void>::type>
    Kmer& operator=(IterT iter) { fromChars(iter); return *this;}

    template <typename ViewT, typename = typename std::enable_if<has_length<ViewT>::value, void>::type>
    Kmer& operator=(ViewT& v) { fromChars(v); return *this; }
    ...
    /**
     *  This is the same as the above function, but it will be called if the argument type 
     *  `ViewT` has a "length()" member.  In that case, the function will additionally check 
    ...
    /**
     *  This is a convenience function taht lets us call fromChars on a string, or string_vew (or similar object);
     **/
    template <typename ViewT, typename = typename std::enable_if<has_length<ViewT>::value, void>::type>
    ...
    /**
     * @returns a `uint64_t` that represents the encoded `idx`-th word of this k-mer
     **/
    ...
    /**
     * @returns a reference to the `uint64_t` that represents the encoded `idx`-th word of this k-mer
     **/
    uint64_t& word__(uint32_t idx) {
        return data_[idx];
    }
    ...
    uint64_t sizeInWords() const {
        return sizeof(data_) / sizeof(base_type);
    }

    /**
     *  @returns the number of words required by this k-mer
     **/
    uint64_t nb_words() const {
        return sizeInWords();
    }
    ...
    void rc() {
        data_[0] = word_reverse_complement(data_[0], k_);
    }
...
#endif // __STAND_ALONE_KMER_HPP__
